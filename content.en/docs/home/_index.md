...
It prioritizes the current VPN state, selected node, start/stop control, traffic entry, location entry, and a single node list. Full node details are shown on the Node Info page instead of occupying the Home workspace.
...
| Full Config | A structured local node with its own outbounds, routing, and DNS, based on the current Xray Profile at runtime. FakeDNS is managed by the current Xray Profile. | Home Local group only. |
| Raw Json | A complete Xray JSON document imported and edited as text. | Home Local group only. |
| Xray Profile | A structured reusable Xray configuration generated by OneXray UI pages. | Core > Xray Profiles. |
| Simple | A built-in Xray Profile writer. | Core > Xray Profiles > Simple. |
...
Home no longer has separate Outbound and JSON tabs. Local Outbound nodes, Full Config nodes, and Raw Json configs are shown together under `Local`; subscription groups contain only subscription Outbound nodes. Full Config and Raw Json are not created by subscriptions.
