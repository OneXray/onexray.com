...
Ordinary Custom routes store and export their local DNS address using the standard `dns.servers` entry tagged `app-dns-direct`. Runtime-only direct-domain matching is generated by the app. See the [custom-route JSON example]({{< relref "/docs/connect/custom-routing" >}}).
