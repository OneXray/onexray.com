...
Enable FakeDNS to return virtual IPs for eligible DNS queries and restore the domain before routing. Domains selected by Local DNS still use real IPs. FakeDNS is off by default and does not affect All traffic through VPN. Read the [DNS cache and system-route limitations]({{< relref "/docs/configuration/dns" >}}) before enabling it.
