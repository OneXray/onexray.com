...
Home больше не использует отдельные вкладки Outbound и JSON. Локальные Outbound nodes и все Raw Json configs показываются вместе в группе `Local`; группы подписок содержат только subscription Outbound nodes. Raw Json не создается из подписок.
