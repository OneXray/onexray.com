...
Импорт: Подключение → режим трафика → собственный маршрут → обычная конфигурация, не импорт серверов. Выберите профиль и уже имеющиеся узлы. Глава описывает только формат обычной формы. Для собственных DNS, sniffing и дополнительных inbounds используйте отдельный [продвинутый JSON-шаблон]({{< relref "/docs/configuration/advanced-routing" >}}). Оба типа делят лимит в три профиля и не конвертируются при редактировании.
