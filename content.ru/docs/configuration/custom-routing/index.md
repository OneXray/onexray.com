...
| dns | Необязательно; только фиксированный tagged Local DNS ниже |
...
Базовые условия: domain и ip — массивы строк; port, например `"443"`, `"8000-8080"`, `"80,443"`; network — tcp, udp или tcp,udp. Имя правила — ruleTag.
...
При наличии dns укажите ровно один `{"tag":"app-dns-direct","address":"8.8.8.8"}`. Меняйте address, не tag. Не добавляйте domains, queryStrategy, skipFallback или отдельный port. Порт включается в поддерживаемую строку адреса, например tcp://192.168.50.53:5353.
...
Не генерируйте эти поля для 26.9.2 или неизвестной сборки.
...
FakeDNS сохраняется дополнительным tagged DNS server. Сохраните direct server; не экспортируйте пулы или sniffing и не добавляйте корневой boolean.
