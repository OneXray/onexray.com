...
| inbounds с tag tunIn | Приложение заменяет/создаёт; не включайте в пользовательский шаблон |
...
Редкие поля описаны в [справочнике Xray соответствующей версии](https://xtls.github.io/config/), который не отменяет границы приложения. Если Raw объявляет сервер или пул FakeDNS, приложение включает восстановление на управляемом inbound; см. [поведение и ограничения FakeDNS]({{< relref "/docs/configuration/dns" >}}).
