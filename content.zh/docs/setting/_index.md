...
Xray-core 相关页面已移到 [Core]({{< relref path="../core/_index.md" lang="zh" >}})。TUN、Ping、Logs、Xray 配置和 GeoData 不再作为 Settings 根页面展示。
...
这个操作会保留本地备份 ZIP 和应用偏好，例如隐私接受状态、first-run 状态、简易配置、TUN 设置、主题、语言、Dock 设置、query-all-packages 授权和 iOS 原生 App 图标。清理后当前 Xray 配置会重置为 `Simple`。
